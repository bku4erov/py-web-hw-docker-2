...
Например:
```bash
docker build . --tag=py-web-hw-docker-2
```
...
Например:
```bash
docker run -d -p 8000:8000 "py-web-hw-docker-2"
```

После запуска REST API будет доступно на заданном порту локального хоста.

### Оставить запущенный docker-контейнер:
```bash
docker stop <container id or name>
```
Идентификатор или имя контейнера можно узнать через просмотр списка запущенных контейнеров:
```bash
docker ps
```
